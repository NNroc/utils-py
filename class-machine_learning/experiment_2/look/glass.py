...
# 读取训练集
train_data = pd.read_csv('glass.data.csv', sep=',')  # 根据你的文件格式修改分隔符
feature_train = train_data.iloc[:, :-1]  # 特征，所有行，去掉最后一列
target_train = train_data.iloc[:, -1]   # 目标变量，所有行，只有最后一列

# 读取测试集
test_data = pd.read_csv('glass.test.csv', sep=',')  # 根据你的文件格式修改分隔符
feature_test = test_data.iloc[:, :-1]  # 特征，所有行，去掉最后一列
target_test = test_data.iloc[:, -1]   # 目标变量，所有行，只有最后一列
...
for i in range(len(classes)):
    ...
    for clf in classes[i]:
        cv_scores = cross_val_score(clf, feature_train, target_train, cv=5)  # 5折交叉验证
...
for name, clf in classifiers6.items():
    clf.fit(feature_train, target_train)  # 训练模型
    y_pred = clf.predict(feature_test)  # 进行预测
    accuracy = accuracy_score(target_test, y_pred)  # 计算准确率
